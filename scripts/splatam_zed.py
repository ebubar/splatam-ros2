...
def live_render(
    ...
    cam,
    time_idx,
    win_rgb="SplaTAM Live RGB",
    win_depth="SplaTAM Live Depth",
    show_depth=False,
):
    """
    Renders the current scene estimate from the pose at `time_idx`
    and shows it in OpenCV windows.

    NOTE: This assumes `cam` is the raster_settings object returned by setup_camera.
    """
    with torch.no_grad():
        # Transform gaussians to this frame (no grads)
        ...
        rgb = im.detach().clamp(0, 1).permute(1, 2, 0).cpu().numpy()
        bgr = (rgb[..., ::-1] * 255).astype(np.uint8)
        cv2.imshow(win_rgb, bgr)

        # Optional depth view
        if show_depth:
            # Build w2c for current time_idx from params
            curr_cam_rot = F.normalize(params["cam_unnorm_rots"][..., time_idx].detach())
            curr_cam_tran = params["cam_trans"][..., time_idx].detach()

            curr_w2c = torch.eye(4, device=curr_cam_tran.device, dtype=torch.float32)
            curr_w2c[:3, :3] = build_rotation(curr_cam_rot)
            curr_w2c[:3, 3] = curr_cam_tran

            depth_sil_rendervar = transformed_params2depthplussilhouette(
                params, curr_w2c, transformed_gaussians
            )
            depth_sil, _, _, = Renderer(raster_settings=cam)(**depth_sil_rendervar)
            depth = depth_sil[0].detach().cpu().numpy()

            depth_vis = depth.copy()
            depth_vis[depth_vis <= 0] = 0

            if np.any(depth_vis > 0):
                dmax = float(np.percentile(depth_vis[depth_vis > 0], 95))
            else:
                dmax = 1.0

            depth_vis = np.clip(depth_vis / max(dmax, 1e-6), 0, 1)
            depth_vis = (depth_vis * 255).astype(np.uint8)
            cv2.imshow(win_depth, depth_vis)

        # Keyboard handling (ESC quits)
        k = cv2.waitKey(1) & 0xFF
        if k == 27:
            raise KeyboardInterrupt("ESC pressed - exiting live view.")
...
def rgbd_slam(config: dict):
    ...
    total_frame_time_sum = 0.0
    total_frame_count = 0
    tracking_only_time_sum = 0.0
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        # ---- END tracking timing BEFORE live_render ----
        if device.type == "cuda":
            torch.cuda.synchronize()
        tracking_end_time = time.time()

        tracking_frame_time = tracking_end_time - tracking_start_time
        tracking_frame_time_sum += tracking_frame_time
        tracking_frame_time_count += 1

        tracking_only_time_sum += tracking_frame_time
        tracking_only_count += 1

        tracking_fps = 1.0 / tracking_frame_time if tracking_frame_time > 0 else 0.0
        print(f"[Tracking FPS] Frame {time_idx}: {tracking_fps:.2f}")

        # Live render ONCE per frame after pose is finalized
        if config.get("live_view", {}).get("enabled", False) and not config.get("live_view", {}).get("during_tracking", False):
            live_render(
                params,
                cam if not seperate_tracking_res else tracking_cam,
                time_idx,
                show_depth=config.get("live_view", {}).get("show_depth", False),
            )
        ...
        # ---- accumulate for final average total FPS ----
        total_frame_time_sum += frame_time
        total_frame_count += 1

        fps = 1.0 / frame_time if frame_time > 0 else 0.0
        print(f"[FPS] Frame {time_idx}: {fps:.2f}")
    ...
    # ---- ADD FPS SUMMARY ----
    if tracking_frame_time_avg > 0:
        print(f"Average Tracking FPS: {1.0/tracking_frame_time_avg:.2f}")
    else:
        print("Average Tracking FPS: N/A")

    if mapping_frame_time_avg > 0:
        print(f"Average Mapping FPS: {1.0/mapping_frame_time_avg:.2f}")
    else:
        print("Average Mapping FPS: N/A")
